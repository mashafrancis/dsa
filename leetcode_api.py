...
class LeetCodeAPI:
    ...
    def __get_problem_by_slug(self, slug: str) -> dict:
        ...
        while not res:
            res = self.session.post(url, data=json_data, headers=headers,
                                    timeout=10)
        question: dict = res.json()
        return question["data"]["question"]

    def __get_display_title(
            self, *, problem_no: int, title: str, link: str,
            emoji: str) -> str:
            ...
            with open(f"{self.problems_path}{filled_num}.md", "w") as f:
                ...
                if len(ways) > 1:
                    ...
                    for way, time_complexity, space_complexity in zip(
                        ...
                        f.write("\n")
                        self.__write_code(f, filled_num, matches[0],
                                          approach_index)
                        approach_index += 1
                else:
    ...
    def write_mkdocs(self) -> None:
        ...
        with open("mkdocs/mkdocs.yml", "a+") as f:
            f.write("  - Problems:\n")
            for problem in self.problems:
                frontend_question_id: int = problem["stat"][
    ...
    def write_readme(self) -> None:
        ...
        for i, line in enumerate(original_readme):
            if line.startswith(
                    '<img src="https://img.shields.io/badge/Solved'):
                break
